...
{
    public partial class StudentContact1 : Form
    {
        int StudentContactX, StudentContactY; // Declaring two variables to store the X and Y coordinates of the form.

        public StudentContact1() // Constructor method for initializing the form.
        {
            InitializeComponent(); // Initializes the form’s components like buttons, textboxes, etc.
        }

        private void StudentContact1_Load(object sender, EventArgs e) // This method runs when the form is loaded.
        {
        ...
        {
            StudentContactX -= 10; // Moves the form 10 pixels to the left on the screen.
            this.Location = new Point(StudentContactX, StudentContactY); // Updates the form’s location to the new coordinates.
            if (StudentContactX <= 1625) // If the form has moved far enough to the left.
            {
        ...
        private void AnimationEndTimer_Tick(object sender, EventArgs e) // This method runs when the AnimationEndTimer ticks (when it's time for the form to move right).
        {
            ...
            {
                StudentContactX += 1; // Moves the form a little bit to the right.
                this.Location = new Point(StudentContactX += 10, StudentContactY); // Moves the form 10 pixels to the right.
                if (StudentContactX > 2500) // If the form has moved too far to the right.
                {
        ...
        private void Position() // This method sets the form's initial position on the screen.
        {
            ...
            StudentContactX = ScreenWidth - this.Width + 120; // Sets the initial X position of the form near the right edge of the screen.
            StudentContactY = ScreenHeight - this.Height - 5; // Sets the initial Y position of the form near the bottom of the screen.

            this.Location = new Point(StudentContactX, StudentContactY); // Places the form at the calculated position.
